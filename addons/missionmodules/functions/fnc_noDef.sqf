...

{
    ...
    if (call compile ("isNil " + "'" + _prefix + "NoDef" + "'")) then {

        call compile (_prefix + "NoDef" + " = " + "[]");

    };

    {
        if !(_x isKindOf "Logic") then {
            _x call compile (_prefix + "NoDef" + " pushback " + "(group _this)");
        } else {
            _x setVariable ["_ExtraArgs",(_logic getVariable ["_ExtraArgs",""]) + "; " + _prefix + "NoDef" + " pushback " + "(group _this)"];
        };
