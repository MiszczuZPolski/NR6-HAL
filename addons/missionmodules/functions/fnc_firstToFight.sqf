...

{
    ...
    if (call compile ("isNil " + "'" + _prefix + "FirstToFight" + "'")) then {

        call compile (_prefix + "FirstToFight" + " = " + "[]");

    };

    {
        if !(_x isKindOf "Logic") then {
            _x call compile (_prefix + "FirstToFight" + " pushback " + "(group _this)");
        } else {
            _x setVariable ["_ExtraArgs",(_logic getVariable ["_ExtraArgs",""]) + "; " + _prefix + "FirstToFight" + " pushback " + "(group _this)"];
        };
