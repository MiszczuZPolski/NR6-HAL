...

{
    ...
    if (call compile ("isNil " + "'" + _prefix + "ExMedic" + "'")) then {

        call compile (_prefix + "ExMedic" + " = " + "[]");

    };

    {
        if !(_x isKindOf "Logic") then {
            _x call compile (_prefix + "ExMedic" + " pushback " + "(group _this)");
        } else {
            _x setVariable ["_ExtraArgs",(_logic getVariable ["_ExtraArgs",""]) + "; " + _prefix + "ExMedic" + " pushback " + "(group _this)"];
        };
