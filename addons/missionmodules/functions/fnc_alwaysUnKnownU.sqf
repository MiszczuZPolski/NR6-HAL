...

{
    ...
    if (call compile ("isNil " + "'" + _prefix + "AlwaysUnKnownU" + "'")) then {

        call compile (_prefix + "AlwaysUnKnownU" + " = " + "[]");

    };

    {
        if !(_x isKindOf "Logic") then {
            _x call compile (_prefix + "AlwaysUnKnownU" + " pushback " + "_this");
        } else {
            _x setVariable ["_ExtraArgs",(_logic getVariable ["_ExtraArgs",""]) + "; " + _prefix + "AlwaysUnKnownU" + " pushback " + "_this"];
        };
