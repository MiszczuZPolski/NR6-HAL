...

{
    ...
    if (call compile ("isNil " + "'" + _prefix + "RCAP" + "'")) then {

        call compile (_prefix + "RCAP" + " = " + "[]");
...

    {
        if !(_x isKindOf "Logic") then {
            ...
            _x setVariable ["_ExtraArgs",(_logic getVariable ["_ExtraArgs",""]) + "; " + _prefix + "RCAP" + " pushback " + "(group _this)"];
        };
