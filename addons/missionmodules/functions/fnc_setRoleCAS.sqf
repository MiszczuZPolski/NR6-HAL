...

{
    ...
    private _varName = QGVAR(rCAS) + _letter;
...

    {
        if !(_x isKindOf "Logic") then {
            // BUG FIX (user-authorized): legacy code had _prefix + "NoAttack" here
            // which was a copy-paste bug — should push to RCAS, not NoAttack.
