...

{
    ...
    if (call compile ("isNil " + "'" + _prefix + "AmmoDrop" + "'")) then {

        call compile (_prefix + "AmmoDrop" + " = " + "[]");

    };

    {
        if !(_x isKindOf "Logic") then {
            _x call compile (_prefix + "AmmoDrop" + " pushback " + "(group _this)");
        } else {
            _x setVariable ["_ExtraArgs",(_logic getVariable ["_ExtraArgs",""]) + "; " + _prefix + "AmmoDrop" + " pushback " + "(group _this)"];
        };
