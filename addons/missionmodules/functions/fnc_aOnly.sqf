...

{
    ...
    if (call compile ("isNil " + "'" + _prefix + "AOnly" + "'")) then {

        call compile (_prefix + "AOnly" + " = " + "[]");

    };

    {
        if !(_x isKindOf "Logic") then {
            _x call compile (_prefix + "AOnly" + " pushback " + "(group _this)");
        } else {
            _x setVariable ["_ExtraArgs",(_logic getVariable ["_ExtraArgs",""]) + "; " + _prefix + "AOnly" + " pushback " + "(group _this)"];
        };
