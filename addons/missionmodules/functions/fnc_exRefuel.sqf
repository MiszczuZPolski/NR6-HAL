...

{
    ...
    if (call compile ("isNil " + "'" + _prefix + "ExRefuel" + "'")) then {

        call compile (_prefix + "ExRefuel" + " = " + "[]");

    };

    {
        if !(_x isKindOf "Logic") then {
            _x call compile (_prefix + "ExRefuel" + " pushback " + "(group _this)");
        } else {
            _x setVariable ["_ExtraArgs",(_logic getVariable ["_ExtraArgs",""]) + "; " + _prefix + "ExRefuel" + " pushback " + "(group _this)"];
        };
