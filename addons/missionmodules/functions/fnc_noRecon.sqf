...

{
    ...
    if (call compile ("isNil " + "'" + _prefix + "NoRecon" + "'")) then {

        call compile (_prefix + "NoRecon" + " = " + "[]");

    };

    {
        if !(_x isKindOf "Logic") then {
            _x call compile (_prefix + "NoRecon" + " pushback " + "(group _this)");
        } else {
            _x setVariable ["_ExtraArgs",(_logic getVariable ["_ExtraArgs",""]) + "; " + _prefix + "NoRecon" + " pushback " + "(group _this)"];
        };
