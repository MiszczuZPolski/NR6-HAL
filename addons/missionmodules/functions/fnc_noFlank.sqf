...

{
    ...
    if (call compile ("isNil " + "'" + _prefix + "NoFlank" + "'")) then {

        call compile (_prefix + "NoFlank" + " = " + "[]");

    };

    {
        if !(_x isKindOf "Logic") then {
            _x call compile (_prefix + "NoFlank" + " pushback " + "(group _this)");
        } else {
            _x setVariable ["_ExtraArgs",(_logic getVariable ["_ExtraArgs",""]) + "; " + _prefix + "NoFlank" + " pushback " + "(group _this)"];
        };
