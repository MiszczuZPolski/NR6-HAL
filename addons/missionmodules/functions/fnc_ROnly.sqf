...

{
    ...
    if (call compile ("isNil " + "'" + _prefix + "ROnly" + "'")) then {

        call compile (_prefix + "ROnly" + " = " + "[]");

    };

    {
        if !(_x isKindOf "Logic") then {
            _x call compile (_prefix + "ROnly" + " pushback " + "(group _this)");
        } else {
            _x setVariable ["_ExtraArgs",(_logic getVariable ["_ExtraArgs",""]) + "; " + _prefix + "ROnly" + " pushback " + "(group _this)"];
        };
