...
// Originally from nr6_hal/Boss.sqf -- main AI commander loop
...
if (EGVAR(missionmodules,debug)) then
	...
	waitUntil
		{
		sleep 0.1;
		_ready = _x getVariable [QEGVAR(hac,readyForBB),false];
		(_ready)
		};
	}
forEach _BBHQGrps;
...
*/
	...
	_ForcesRep = [_BBHQGrps] call EFUNC(boss,forceAnalyze);
	...
	if (_BBCycle == 1) then
		...
		if (EGVAR(missionmodules,debug)) then
			...
			{
			_isLeft = ([(getPosATL (vehicle (leader _x))),_ArmyPos,_attackAxis] call EFUNC(boss,whereIs)) select 0;
			_x setVariable ["isLeft",_isLeft]
			}
		forEach _BBHQGrps;

			{
			_isLeft = ([(_x select 0),_ArmyPos,_attackAxis] call EFUNC(boss,whereIs)) select 0;
			_x set [3,_isLeft]
			}
		forEach _strArea;
		...
		_frontSectors = [];
			...
			{
			_where = [(position _x),_ArmyPos,_attackAxis] call EFUNC(boss,whereIs);
		...
		_frontSA = [];
			...
			{
			_where = [(_x select 0),_ArmyPos,_attackAxis] call EFUNC(boss,whereIs);
			_isLeft = _where select 0;
			_isFlank = _where select 1;
			_isRear = _where select 2;
		...
		if (((RydBBa_SimpleDebug) and (_BBSide == "A")) or ((RydBBb_SimpleDebug) and (_BBSide == "B"))) then
			...
			[[_BBHQGrps,_BBSide],EFUNC(boss,BBSimpleD)] call EFUNC(common,spawn)
			};
		};
	...
	if ((_lossesFactor > (0.6 + (random 0.15))) or (_allAreTaken)) then
		...
		_stance = "D";
			...
			if (_aliveHQ) then
				{
				_HQ = _x;
				_front = _HQ getVariable [QEGVAR(common,front),locationNull];
				_tObj1 = _HQ getVariable [QEGVAR(core,obj1),objNull];
				_tObj2 = _HQ getVariable [QEGVAR(core,obj2),objNull];
				_tObj3 = _HQ getVariable [QEGVAR(core,obj3),objNull];
				_tObj4 = _HQ getVariable [QEGVAR(core,obj4),objNull];
				_knEnemy = _HQ getVariable [QGVAR(knEnPos),[]];
				_HQ setVariable [QEGVAR(core,order),"DEFEND"];
				_HQ setVariable [QEGVAR(core,nObj),5];
...

			{
				...
				if (_aliveHQ) then
					...
					if (_pathDone) then
						...
						_tObj3 = _HQ getVariable [QEGVAR(core,obj3),objNull];
						_tObj4 = _HQ getVariable [QEGVAR(core,obj4),objNull];
						_HQ setVariable [QEGVAR(core,order),"ATTACK"];
						...
						_pathRep = [_sctrs,_areas,_HQpos,_acT,_BBSide] call EFUNC(boss,itinerary);
						...
						[[_x,_tgtsAround,_tObj1,_tObj2,_tObj3,_tObj4,_BBHQGrps,_HQpos,_front,_secsAround,_goingReserve,_BBSide,_AAOPts],EFUNC(boss,executePath)] call EFUNC(common,spawn);
	...
	/*
				{
				_mark = [(str (random 1000)),_x,"ColorOrange","ICON",[0.5,0.5],0,1,"DOT","R"] call EFUNC(boss,marker);
				}
			foreach _points;
	*/
			...
			[[_x,_goingAhead,_tObj1,_tObj2,_tObj3,_tObj4,_BBHQs,_front,_takenPoints,_hostileGroups,_BBSide],EFUNC(boss,reserveExecuting)] call EFUNC(common,spawn);
	...
	if (_BBCycle == 1) then
		...
		[[_strArea,_BBSide,(_BBHQGrps select 0),_BBHQGrps],EFUNC(boss,objectivesMon)] call EFUNC(common,spawn);
		};
