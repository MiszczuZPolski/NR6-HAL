...
_airDef = ((_HQ getVariable [QEGVAR(core,airG),[]]) - ((_HQ getVariable [QEGVAR(boss,nCAirG),[]]) + (_HQ getVariable [QEGVAR(core,nCrewInfG),[]]))) - ((_HQ getVariable [QEGVAR(core,noDef),[]]) + (_HQ getVariable [QGVAR(specForG),[]]) + (_HQ getVariable [QEGVAR(core,cargoOnly),[]]) + (_HQ getVariable [QEGVAR(core,ammoDrop),[]]) + (_HQ getVariable [QEGVAR(core,supportG),[]]));
...
if not ((count (_HQ getVariable [QEGVAR(boss,knEnPos),[]])) == 0) then 
	{
		{
		_Epos0 pushBack (_x select 0);
		_Epos1 pushBack (_x select 1)
		}
	forEach (_HQ getVariable [QEGVAR(boss,knEnPos),[]])
	}
else
	{
...
_Angle = 0;
	...
	if ((count (_HQ getVariable [QEGVAR(boss,knEnPos),[]])) == 0) then
		{
