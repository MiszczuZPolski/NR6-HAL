...
// Originally from nr6_hal/Desperation.sqf -- desperation logic
...
_ally = _HQ getVariable [QEGVAR(core,friends),[]];
_enemy = _HQ getVariable [QEGVAR(common,knEnemiesG),[]];
...
if (((_midD > _midDE) and (_midDE < 2000)) or (_nearE)) then
	{
	_HQ setVariable [QEGVAR(core,recklessness),(_rInit + 0.2) * 10]
	}
else
	{
	_HQ setVariable [QEGVAR(core,recklessness),_rInit]
	};
