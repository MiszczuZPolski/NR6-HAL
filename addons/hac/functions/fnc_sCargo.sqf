...
if (isNull _ChosenOne) then
	...
	switch (true) do
		...
		if (((count ((leader _HQ) getVariable [QEGVAR(tasking,transportPriorityAir),[]])) > 0) and not (_withdraw) and not (_request)) then {_cargos = [_GCargo]};
		if (((count ((leader _HQ) getVariable [QEGVAR(tasking,transportPriorityGnd),[]])) > 0) and not (_withdraw) and not (_request)) then {_cargos = [_airCargo]};
...
//		} foreach (units _unitG);
		...
		_TransportPriority = (leader _HQ) getVariable [QEGVAR(tasking,transportPriorityAir),[]];
		_TransportPriority = _TransportPriority - [_unitG];
		(leader _HQ) setVariable [QEGVAR(tasking,transportPriorityAir),_TransportPriority,true];

		_TransportPriority = (leader _HQ) getVariable [QEGVAR(tasking,transportPriorityGnd),[]];
		_TransportPriority = _TransportPriority - [_unitG];
		(leader _HQ) setVariable [QEGVAR(tasking,transportPriorityGnd),_TransportPriority,true];
