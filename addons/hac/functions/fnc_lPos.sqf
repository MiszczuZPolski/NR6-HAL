...
// Originally from nr6_hal/HAL/LPos.sqf
...
_HQ = _this select 0;

	{
		...
		if (not (isPlayer (leader _x)) and (_dst > (_HQ getVariable [QEGVAR(core,airDist),4000]))) then 
			{
...
forEach (_HQ getVariable [QEGVAR(core,airG),[]]);
