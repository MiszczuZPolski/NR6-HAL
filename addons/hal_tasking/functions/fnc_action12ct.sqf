 ...
 */
	...
	[_unit, 'Command, requesting aerial medical support - Over'] remoteExecCall ["RYD_MP_Sidechat"];
	...
	if (_MedBoy isEqualTo objNull) exitWith {[leader _HQ, (groupId _grp) + ', negative. No MEDEVAC helicopters are currently available - Out'] remoteExecCall ["RYD_MP_Sidechat"]};

	[[_MedBoy,(vehicle _unit),[],_HQ,true],HAL_GoMedSupp] call EFUNC(common,spawn);

	[leader _HQ, (groupId _grp) + ', affirmative. Helicopter is on its way - Out'] remoteExecCall ["RYD_MP_Sidechat"];
