...
// Originally from nr6_hal/TaskInitNR6.sqf:968 (Action11ct)
/**
 * @description Server-side condition/execution handler for slot 11: Request ambulance (ground medical)
 ...
 */
	...
	[_unit, 'Command, requesting ambulance - Over'] remoteExecCall ["RYD_MP_Sidechat"];

	sleep 5;

	private _Pool = RHQ_Med + RYD_WS_med - RHQs_Med;
	...
	if (_MedBoy isEqualTo objNull) exitWith {[leader _HQ, (groupId _grp) + ', negative. No ambulances are currently available - Out'] remoteExecCall ["RYD_MP_Sidechat"]};

	[[_MedBoy,(vehicle _unit),[],_HQ,true],HAL_GoMedSupp] call EFUNC(common,spawn);

	[leader _HQ, (groupId _grp) + ', affirmative. Ambulance is on its way - Out'] remoteExecCall ["RYD_MP_Sidechat"];
