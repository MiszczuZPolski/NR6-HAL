 ...
 */
	...
	[_unit, 'Command, requesting ambulance - Over'] remoteExecCall ["hal_common_fnc_MP_Sidechat"];
...

	{
		...
		if ( not ((group _x) getVariable [("Busy" + _unitvar),false]) and not ((group _x) == _grp) and not ((group _x) getVariable ["Unable",false]) and not ((group _x) isEqualTo grpNull) and (canMove _x) and ((toLower (typeOf (assignedVehicle (leader (group _x))))) in _Pool) and not (_x in (_HQ getVariable [QEGVAR(core,airG),[]])) and not ((group _x) in ((_HQ getVariable [QEGVAR(hal_hac,specForG),[]]) + (_HQ getVariable [QEGVAR(core,cargoOnly),[]]))))  exitWith {_MedBoy = _x};
	} forEach (_HQ getVariable [QEGVAR(hal_hac,support),[]]);
