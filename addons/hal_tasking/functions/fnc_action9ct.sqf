 ...
 */
	...
	[_unit, 'Command, requesting ammunition truck - Over'] remoteExecCall ["RYD_MP_Sidechat"];
	...
	if (_AmmoBoy isEqualTo objNull) exitWith {[leader _HQ, (groupId _grp) + ', negative. No rearming services are currently available - Out'] remoteExecCall ["RYD_MP_Sidechat"]};

	[[_AmmoBoy,(vehicle _unit),[],[],false,objNull,_HQ,true],HAL_GoAmmoSupp] call EFUNC(common,spawn);

	[leader _HQ, (groupId _grp) + ', affirmative. Ammunition truck is on its way - Out'] remoteExecCall ["RYD_MP_Sidechat"];
