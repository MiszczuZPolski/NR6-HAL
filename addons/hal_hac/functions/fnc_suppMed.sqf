...
_med = EGVAR(hal_data,med) + EGVAR(hal_data,wS_med) - RHQs_Med;
_noenemy = true;
...
_medASG = [];
	...
	if not (_x in _medS) then
		...
		if ((toLower (typeOf (assignedVehicle _x))) in _med) then 
			...
			if not ((group _x) in (_medASG + (_HQ getVariable [QEGVAR(hal_hac,specForG),[]]) + (_HQ getVariable [QEGVAR(core,cargoOnly),[]]))) then
				{
