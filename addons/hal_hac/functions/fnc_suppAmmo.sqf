...
_ammo = EGVAR(hal_data,ammo) + EGVAR(hal_data,wS_ammo) - RHQs_Ammo;
...
_ammoSG = [];
	...
	if not (_x in _ammoS) then
		...
		if ((toLower (typeOf (assignedVehicle _x))) in _ammo) then 
			...
			if not ((group _x) in (_ammoSG + (_HQ getVariable [QEGVAR(hal_hac,specForG),[]]) + (_HQ getVariable [QEGVAR(core,cargoOnly),[]]))) then 
				{
