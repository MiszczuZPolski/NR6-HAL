...
// Originally from nr6_hal/HAL/SCargo.sqf
_SCRname = "SCargo";
...
if ((_withdraw) and not (_HQ getVariable [QEGVAR(core,airEvac),true])) exitWith {_unitG setVariable ["CargoChosen",false,true];_unitG setVariable [("CC" + (str _unitG)), true, true]};

_enmyNrb = false;
_enmyNrb = (([(leader _unitG),(_HQ getVariable [QEGVAR(common,knEnemiesG),[]]),300] call EFUNC(common,closeEnemyB)) select 0);
...
if (isNull _ChosenOne) then
	...
	switch (true) do
		...
		if (((count ((leader _HQ) getVariable [QEGVAR(hal_tasking,transportPriorityAir),[]])) > 0) and not (_withdraw) and not (_request)) then {_cargos = [_GCargo]};
		if (((count ((leader _HQ) getVariable [QEGVAR(hal_tasking,transportPriorityGnd),[]])) > 0) and not (_withdraw) and not (_request)) then {_cargos = [_airCargo]};

		{
			...
			forEach (units _x);
				...
				if ((group _x) in (_HQ getVariable [QEGVAR(core,airG),[]])) then {_mpl = 100} else {_mpl = 1};
				_noenemy = true;
									...
									((fuel (assignedVehicle _x)) >= 0.2) and
										(damage (assignedVehicle _x) <= 0.8) and
											(canMove (assignedVehicle _x)) and
												(_noenemy) and
													(not ((group _x) in (_HQ getVariable [QEGVAR(core,airG),[]])) or (((count ((_HQ getVariable [QEGVAR(core,aAthreat),[]]) + (_HQ getVariable [QEGVAR(core,airthreat),[]]))) == 0) or (random 100 > (85/(0.5 + (2*(_HQ getVariable [QEGVAR(core,recklessness),0.5])))))))) exitWith
					{
...
_unitvar = str _unitG;
if (isNull _ChosenOne) exitWith {{[_x] remoteExecCall [QEFUNC(common,MP_unassignVehicle),0]} forEach (units _unitG);_unitG setVariable [("CC" + _unitvar), true, true];_unitG setVariable ["CargoChosen",false,true];_unitG setVariable ["AssignedCargo" + (str _unitG),objNull,true];_unitG setVariable ["CargoCheckPending" + (str _unitG),false];};
...
if ((_ChosenOne isKindOf "Air") and not (_withdraw)) then
		{
			_enmyNrb = (([(leader _unitG),(_HQ getVariable [QEGVAR(common,knEnemiesG),[]]),600] call EFUNC(common,closeEnemyB)) select 0);
		} else {
			_enmyNrb = (([(leader _unitG),(_HQ getVariable [QEGVAR(common,knEnemiesG),[]]),300] call EFUNC(common,closeEnemyB)) select 0);
		};
if ((_enmyNrb) and not (_request)) exitWith {_unitG setVariable ["CargoChosen",false,true];_unitG setVariable [("CC" + (str _unitG)), true, true]};
...
if not (_emptyV) then
	...
	if (_offRoad) then
		...
		if not (_GD in _airCargo) then
			...
			_GD setVariable [QGVAR(eDPos),_EDpositions];
			};
		};

	if not (_alive) exitWith {{[_x] remoteExecCall [QEFUNC(common,MP_unassignVehicle),0]} forEach (units _unitG);_unitG setVariable [("CC" + _unitvar), true, true];_unitG setVariable ["CargoChosen",false,true];_unitG setVariable ["AssignedCargo" + (str _unitG),objNull,true];_unitG setVariable ["CargoCheckPending" + (str _unitG),false];};
...
//	if (_request) then {_GD setvariable ["HAL_ReqTraOn",false,true]};
	...
	[_GD] call CBA_fnc_clearWaypoints;
	...
	if not (_withdraw) then {
		_task = [(leader _GD),["Pick up "+ (groupId _unitG) + " for transport to a designated position.", "Pick Up " + (groupId _unitG) , ""],_Lpos,"run"] call EFUNC(common,addTask);
		} else {
		_task = [(leader _GD),["Pick up " + (groupId _unitG) + " for MEDEVAC.", "MEDEVAC " + (groupId _unitG), ""],_Lpos,"heal"] call EFUNC(common,addTask);
		};
...
/*	if (_withdraw) then
		...
		_angle = [_pos1,_pos2,5] call EFUNC(common,angleTowards);
		_posLZ = [_pos1,_angle,100] call EFUNC(common,positionTowards2D);
		...
		while {_dstE < 400} do
			{
			_posLZ = [_pos1,_angle,_dst] call EFUNC(common,positionTowards2D);
			_dst = _dst + 100;
		...
		_lz = [_posLZ] call EFUNC(common,LZ);
	...
	if (_ChosenOne isKindOf "Air") then
		...
		if ((_HQ getVariable [QEGVAR(core,lZ),false]) and not (_withdraw)) then
			{
			_lz = [_Lpos] call EFUNC(common,LZ);
	...
	if not (_request) then {
		...
		_task2 = [(leader _unitG),[_taskTxt, "Wait For Lift", ""],_Lpos,"getin"] call EFUNC(common,addTask);

		_wp = [_GD,_Lpos,"MOVE","STEALTH","YELLOW","FULL",["true","{(vehicle _x) land 'GET IN'} foreach (units (group this));deletewaypoint [(group this), 0];"],true,0,[0,0,0],"COLUMN"] call EFUNC(common,WPadd);
...
//		if not (_ChosenOne isKindOf "Air") then {_wp waypointAttachVehicle (vehicle (leader _unitG))};
	...
	waitUntil
		...
		if (_alive) then {if ((_enmyNrb) and not (_request)) then {_endThis = true;_alive = false; [leader _GD, (groupId _unitG) + ', negative. ' + _PUType + ' is too hot at this time - Over'] remoteExecCall [QGVAR(mP_Sidechat)]}};
	...
	if not (_alive) exitWith {
		[_unitG,_ChosenOne] remoteExecCall ["leaveVehicle"];{[_x] remoteExecCall [QEFUNC(common,MP_unassignVehicle),0]} forEach (units _unitG);
		...
		_task = [(leader _GD),["Return to departure base.", "Abort Pick Up, RTB", ""],_LandPos,"land"] call EFUNC(common,addTask);
		...
		_beh = "AWARE";
		if (_GD in (_HQ getVariable [QGVAR(rAirG),[]])) then {_beh = "SAFE"};

		_radd = "";
		if (_rrr) then {_radd = "; {(vehicle _x) setFuel 1; (vehicle _x) setVehicleAmmo 1; (vehicle _x) setDamage 0;} foreach (units (group this))"};

		_wp = [_GD,_LandPos,"MOVE",_beh,"YELLOW","FULL",["true","if not ((group this) getVariable ['AirNoLand',false]) then {{(vehicle _x) land 'LAND'} foreach (units (group this))}; deletewaypoint [(group this), 0]" + _radd],true,0,[0,0,0],"COLUMN"] call EFUNC(common,WPadd);

		if not (isNull (_GD getVariable ["tempLZ",objNull])) then {deleteVehicle (_GD getVariable ["tempLZ",objNull])};

		_GD setVariable [("Busy" + (str _GD)), false];
		_ChosenOne enableAI "TARGET";_ChosenOne enableAI "AUTOTARGET";
		_UL = leader _GD;if not (isPlayer _UL) then {if ((random 100) < EGVAR(core,aIChatDensity)) then {[_UL,GVAR(aIC_OrdEnd),"OrdDen"] call EFUNC(common,AIChatter)}};
		};
	...
	if (_ct > 300) then {_alive = false;[_unitG,_ChosenOne] remoteExecCall ["leaveVehicle"];{[_x] remoteExecCall [QEFUNC(common,MP_unassignVehicle),0]} forEach (units _unitG);_unitG setVariable [("CC" + _unitvar), true, true];_unitG setVariable ["CargoChosen",false,true];_unitG setVariable ["AssignedCargo" + (str _unitG),objNull,true];};
	};
...
if not (_alive) exitWith {
	...
	_task = [(leader _GD),["Return to departure base.", "Return To Base", ""],_LandPos,"land"] call EFUNC(common,addTask);
	...
	_GD setVariable [("Busy" + (str _GD)), false];
	_ChosenOne enableAI "TARGET";_ChosenOne enableAI "AUTOTARGET";
	_UL = leader _GD;if not (isPlayer _UL) then {if ((random 100) < EGVAR(core,aIChatDensity)) then {[_UL,GVAR(aIC_OrdEnd),"OrdEnd"] call EFUNC(common,AIChatter)}};
...
if not (_GD == _unitG) then
	...
	if (_request) then {
		...
		[_ChosenOne,leader _unitG,_GD,not (_requestG)] remoteExecCall [QGVAR(reqTransport_Actions),(leader _unitG)];
	};
	...
	waitUntil
		...
		if (_request) then {
			...
			if (not ((leader _unitG) isEqualTo _firstlead) or (isNull _firstlead)) then {
				...
				if not (isNull _firstlead) then {[_ChosenOne,leader _unitG,_GD,not (_requestG)] remoteExecCall [QGVAR(reqTransport_Actions),(leader _unitG)]};
			...
			if not (isNil "_Rdest") then {
				...
				if (_GD in (_HQ getVariable [QEGVAR(core,airG),[]])) then
				{
				...
				_wp = [_GD,_Rdest,"MOVE",behaviour (leader _GD),combatMode _GD,"NORMAL",["true","(vehicle this) land 'GET IN';deletewaypoint [(group this), 0];"],true,0,[0,0,0],"COLUMN"] call EFUNC(common,WPadd);
...
//		{
//			[_x] remoteExecCall ["EFUNC(common,MP_unassignVehicle)",0];
//		} foreach (units _unitG);

		_TransportPriority = (leader _HQ) getVariable [QEGVAR(hal_tasking,transportPriorityAir),[]];
		_TransportPriority = _TransportPriority - [_unitG];
		(leader _HQ) setVariable [QEGVAR(hal_tasking,transportPriorityAir),_TransportPriority,true];

		_TransportPriority = (leader _HQ) getVariable [QEGVAR(hal_tasking,transportPriorityGnd),[]];
		_TransportPriority = _TransportPriority - [_unitG];
		(leader _HQ) setVariable [QEGVAR(hal_tasking,transportPriorityGnd),_TransportPriority,true];
	...
	if not (_alive) exitWith {
		...
		if not ((_GD == _unitG) or (isNull ((group (assignedDriver _ChosenOne))))) then
			...
			{
				[_x] remoteExecCall [QEFUNC(common,MP_unassignVehicle),0];
		...
		_ChosenOne enableAI "TARGET";_ChosenOne enableAI "AUTOTARGET";
		_UL = leader _GD;if not (isPlayer _UL) then {if ((random 100) < EGVAR(core,aIChatDensity)) then {[_UL,EGVAR(common,aIC_OrdDen),"OrdDen"] call EFUNC(common,AIChatter)}};
	...
	if ((_timer > 600) and not (isNull _GD)) then
		...
		if (_ChosenOne isKindOf "Air") then
			...
			if (_HQ getVariable [QEGVAR(core,lZ),false]) then
				{
				if not (isNull (_GD getVariable ["tempLZ",objNull])) then {deleteVehicle (_GD getVariable ["tempLZ",objNull])};
				_lz = [position _ChosenOne] call EFUNC(common,LZ);
				_GD setVariable ["TempLZ",_lz];
				}
			};
		...
		_GD setVariable [("CargoM" + _unitvar), false];
		_cause = [_unitG,1,false,0,240,[],false,true,false] call EFUNC(common,wait);
		if not (isNull (_GD getVariable ["tempLZ",objNull])) then {deleteVehicle (_GD getVariable ["tempLZ",objNull])};
