...
// Originally from nr6_hal/HAL/HQOrdersDef.sqf
...
_defRange = _HQ getVariable [QEGVAR(core,defRange),1];

_LMCU = ((_HQ getVariable [QEGVAR(core,friends),[]]) - (((_HQ getVariable [QEGVAR(core,airG),[]]) - (_HQ getVariable [QEGVAR(core,nCrewInfG),[]])) + (_HQ getVariable [QGVAR(specForG),[]]) + (_HQ getVariable [QEGVAR(core,cargoOnly),[]]) + (_HQ getVariable [QEGVAR(core,navalG),[]])  + (_HQ getVariable [QEGVAR(core,supportG),[]]) + (_HQ getVariable [QGVAR(staticG),[]]) + (_HQ getVariable [QEGVAR(core,artG),[]]) + ((_HQ getVariable [QEGVAR(core,nCCargoG),[]]) - (_HQ getVariable [QEGVAR(core,nCrewInfG),[]])))) - (_HQ getVariable [QEGVAR(core,noDef),[]]);
_airDef = ((_HQ getVariable [QEGVAR(core,airG),[]]) - ((_HQ getVariable [QEGVAR(hal_boss,nCAirG),[]]) + (_HQ getVariable [QEGVAR(core,nCrewInfG),[]]))) - ((_HQ getVariable [QEGVAR(core,noDef),[]]) + (_HQ getVariable [QGVAR(specForG),[]]) + (_HQ getVariable [QEGVAR(core,cargoOnly),[]]) + (_HQ getVariable [QEGVAR(core,ammoDrop),[]]) + (_HQ getVariable [QEGVAR(core,supportG),[]]));
_recDef = ((_HQ getVariable [QGVAR(reconG),[]]) + (_HQ getVariable [QGVAR(fOG),[]]) + (_HQ getVariable [QGVAR(snipersG),[]])) - ((_HQ getVariable [QEGVAR(core,noDef),[]]) + (_HQ getVariable [QGVAR(specForG),[]]) + (_HQ getVariable [QEGVAR(core,cargoOnly),[]]) + (_HQ getVariable [QEGVAR(core,supportG),[]]));

_LMCU = [_LMCU] call EFUNC(common,randomOrd);
_airDef = [_airDef] call EFUNC(common,randomOrd);
_recDef = [_recDef] call EFUNC(common,randomOrd);
...
forEach ((_LMCU + _airDef + _recDef) - ([_HQ] + (_HQ getVariable [QEGVAR(core,garrison),[]])));
...
if ((_HQ getVariable [QEGVAR(core,firstEMark),true]) and ((_HQ getVariable [QEGVAR(core,lastE),0]) == 0) and ((count (_HQ getVariable [QEGVAR(core,knEnemies),[]])) > 0)) then {_HQ setVariable [QEGVAR(core,firstEMark),false]};

_default = [];
_Epos0 = [];
_Epos1 = [];

_AAO = _HQ getVariable [QEGVAR(hal_boss,chosenAAO),false];
...
_default = getPosATL (_HQ getVariable [QEGVAR(core,obj),(leader _HQ)]);

if (_AAO) then
	{
	_default = _HQ getVariable [QEGVAR(core,eyeOfBattle),getPosATL (vehicle (leader _HQ))]
	};

if not ((count (_HQ getVariable [QEGVAR(hal_boss,knEnPos),[]])) == 0) then 
	{
		{
		_Epos0 pushBack (_x select 0);
		_Epos1 pushBack (_x select 1)
		}
	forEach (_HQ getVariable [QEGVAR(hal_boss,knEnPos),[]])
	}
else
	{
	if not (isNull (leader _HQ)) then 
		...
		{
		_Epos0 = [(position (leader ((_HQ getVariable [QEGVAR(core,friends),[]]) select (random (floor (count (_HQ getVariable [QEGVAR(core,friends),[]]))))))) select 0];
		_Epos1 = [(position (leader ((_HQ getVariable [QEGVAR(core,friends),[]]) select (random (floor (count (_HQ getVariable [QEGVAR(core,friends),[]]))))))) select 1]
		}
	};
...
if ((_HQ getVariable [QEGVAR(core,cRDefRes),0]) == 0) then {

		{
		if ((not (_x in ((_HQ getVariable [QEGVAR(core,nCCargoG),[]]) + (_HQ getVariable [QEGVAR(core,airG),[]]))) or ((count (units _x)) > 1)) and ((random 100) > (70/(0.75 + ((_HQ getVariable [QEGVAR(core,fineness),0.5])/4))))) then 
			{
			_defRes pushBack _x
			};
		}
	forEach _LMCU;
		...
		{
		if ((not (_x in ((_HQ getVariable [QEGVAR(core,nCCargoG),[]]) + (_HQ getVariable [QEGVAR(core,airG),[]]))) or ((count (units _x)) > 1)) and ((random 1) <= (_HQ getVariable [QEGVAR(core,cRDefRes),0]))) then 
			{
			_defRes pushBack _x
			};
		}
...
_HQ setVariable [QGVAR(defRes),_defRes];
...
if ((_HQ getVariable [QEGVAR(core,defendObjectives),0]) > 0) then 
	{
	_defPoints = _defPoints + (_HQ getVariable [QEGVAR(common,taken),[]]); 
...
while {(_ct < 3)} do
	...
	forEach _defPoints;
		...
		if (_x in ((_HQ getVariable [QGVAR(reconG),[]]) + (_HQ getVariable [QGVAR(fOG),[]]) + (_HQ getVariable [QGVAR(snipersG),[]]))) then {_closest setVariable ["ClosestForRec",_clr + 1]}
		}
	forEach _LMCU;
...
_Angle = 0;
	...
	if ((count (_HQ getVariable [QEGVAR(hal_boss,knEnPos),[]])) == 0) then
		{
	...
	switch (_defPoint) do
		...
		case (_HQ getVariable ["leaderHQ",leader _HQ]) : 
			{
			_dL = _HQ getVariable QEGVAR(core,defFrontL);
			if not (isNil "_dL") then {_defFront = _dL}
			};
			
		case (_HQ getVariable [QEGVAR(core,obj1),leader _HQ]) : 
			{
			_d1 = _HQ getVariable QEGVAR(core,defFront1);
			if not (isNil "_d1") then {_defFront = _d1}
			};
			
		case (_HQ getVariable [QEGVAR(core,obj2),leader _HQ]) : 
			{
			_d2 = _HQ getVariable QEGVAR(core,defFront2);
			if not (isNil "_d2") then {_defFront = _d2}
			};
			
		case (_HQ getVariable [QEGVAR(core,obj3),leader _HQ]) : 
			{
			_d3 = _HQ getVariable QEGVAR(core,defFront3);
			if not (isNil "_d3") then {_defFront = _d3}
			};
			
		case (_HQ getVariable [QEGVAR(core,obj4),leader _HQ]) : 
			{
			_d4 = _HQ getVariable QEGVAR(core,defFront4);
	...
	_HQ setVariable [QGVAR(angle),_Angle];
	...
	if not ((count _defFront) == 0) then
		...
		_HQ setVariable [QGVAR(angle),_defDir];
		if (_isHQ) then
			{
			_HQ setVariable [QGVAR(angleR),_defDir];
			}
		};
	...
	_spotsN = _clr * 2;
	_goodSpotsRec = [_spotsN,_PrimDir,_SecDir,_FreeLOS,_Lenght1,_Width1,_Center,_HQ] call FUNC(spotScan);
	...
	_goodSpots = [_spotsN,_PrimDir,_SecDir,_FreeLOS,_Lenght2,_Width2,_Center,_HQ] call FUNC(spotScan);

	_angleV = _HQ getVariable [QGVAR(angle),0];
		...
		if (({alive _x} count (units _x)) > 0) then
			...
			if (not (_isDef) and not (_Unable)) then
				...
				if not (_x in (_HQ getVariable [QEGVAR(core,garrison),[]])) then
					...
					if (_ammo > 0) then
						{
						if (not (_x getVariable ["Busy" + (str _x),false]) and not (_x in (_HQ getVariable [QEGVAR(core,recDefSpot),[]]))) then
							{
...
switch ((random 100) >= (50/(0.5 + (_HQ getVariable [QEGVAR(core,fineness),0.5])))) do
	{
	case true : 
		{			
			{
				...
				if (({alive _x} count (units _x)) > 0) then
					...
					if (not (_isDef) and not (_Unable)) then
						...
						if not (_x in (_HQ getVariable [QEGVAR(core,garrison),[]])) then
							...
							if (_ammo > 0) then
								{
								if (not (_x getVariable ["Busy" + (str _x),false]) and not (_x in (_HQ getVariable [QEGVAR(core,defSpot),[]]))) then
									{
									_closestArr = _defArray select 0;
									_friend =  vehicle (leader _x);
									_dstM = _friend distance (_closestArr select 0);
			...
			//if (false) then
				...
				if (({alive _x} count (units _ad)) > 0) then
					...
					if (not (_isDef) and not (_Unable)) then
						...
						if (_ammo > 0) then
							{
							if ( not (_ad getVariable ["Busy" + (str _ad),false]) and not (_ad in (_HQ getVariable [QEGVAR(core,airInDef),[]]))) then
								{
								_closestArr = _defArray select 0;
					...
					if (not (_isDef) and not (_Unable)) then
						...
						if not (_x in []) then 
							...
							if not (_x in (_HQ getVariable [QEGVAR(core,garrison),[]])) then
								...
								if (_ammo > 0) then
									{
									if not ((_x getVariable ["Busy" + (str _x),false]) and (_x in ((_HQ getVariable [QEGVAR(core,defSpot),[]]) + (_HQ getVariable [QEGVAR(core,def),[]])))) then
										{
										_closestArr = _defArray select 0;
										_friend =  vehicle (leader _x);
										_dstM = _friend distance (_closestArr select 0);
										...
										switch (true) do 
											{
											case (_x in (_HQ getVariable [QGVAR(infG),[]])) : {_expression = "(1 + (2 * Houses)) * (1 + (1.5 * Forest)) * (1 + Trees) * (1 - Meadow) * (1 - (10 * sea))"};
											case (not (_x in (_HQ getVariable [QGVAR(infG),[]]))) : {_expression = "(1 + (2 * Meadow)) * (1 - Forest) * (1 - (0.5 * Trees)) * (1 - (10 * sea))"};
											};
										...
										_Spot = _Spot select 0;
										if ((random 100) > 70/(0.75 + ((_HQ getVariable [QEGVAR(core,fineness),0.5])/2))) then 
											{
		...
		forEach ((_LMCU - ((_HQ getVariable [QEGVAR(core,defSpot),[]]) + (_HQ getVariable [QEGVAR(core,exhausted),[]]) + (_HQ getVariable [QEGVAR(core,recDefSpot),[]]) + (_HQ getVariable [QGVAR(defRes),[]])) + (_HQ getVariable [QEGVAR(core,nCCargoG),[]])) - (_HQ getVariable [QEGVAR(core,noDef),[]]));
		};
	...
	case false : 
		...
		forEach ((_LMCU - ((_HQ getVariable [QEGVAR(core,recDefSpot),[]]) + (_HQ getVariable [QEGVAR(core,exhausted),[]]) + (_HQ getVariable [QGVAR(defRes),[]])) + (_HQ getVariable [QEGVAR(core,nCCargoG),[]])) - (_HQ getVariable [QEGVAR(core,noDef),[]]));
...
forEach ((_HQ getVariable [QGVAR(defRes),[]]) - (_HQ getVariable [QEGVAR(core,noDef),[]]) - (_HQ getVariable [QEGVAR(core,exhausted),[]]));
	...
	if (not (_busy) and not (_Unable) and ((count (waypoints _x)) <= 1) and not (_deployed) and not (_isDef) and not (_capturing) and not ((_HQ getVariable [QGVAR(takenNaval),[]]) isEqualTo []) and (not (_x in (_HQ getVariable [QEGVAR(core,nCCargoG),[]])) or ((count (units _x)) > 1))) then 
		{
