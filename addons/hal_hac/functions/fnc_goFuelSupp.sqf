...
// Originally from nr6_hal/HAL/GoFuelSupp.sqf
...
_FuelPoints = _HQ getVariable [QEGVAR(core,fuelPoints),[]];
...
[_unitG,[_posX,_posY,0],"HQ_ord_fuelS",_HQ] call EFUNC(common,orderPause);
...
if (_HQ getVariable [QEGVAR(common,debug),false]) then
	...
	_i = [[_posX,_posY],_unitG,"markFuelSupp","ColorKhaki","ICON","waypoint","REFUEL " + (groupId _unitG) + " " + _signum," - REFUEL",[0.5,0.5]] call EFUNC(common,mark)
	};

_task = [(leader _unitG),["Deliver Fuel to " + (groupId (group _Trg)), "Refuel " + (groupId (group _Trg)), ""],(position _Trg),"refuel"] call EFUNC(common,addTask);
...
if (_request) then {
	...
	[_cis,"Fuel"] remoteExecCall ["hal_common_fnc_reqLogisticsActions"];
...
//		if (((_cis distance _Trg) < 50) and ((fuel _Trg) == 0)) then {_Trg setfuel 0.09};
	...
	if not (_alive) exitWith
		...
		_HQ setVariable [QEGVAR(core,fuelPoints),_FuelPoints];
	...
	if ((EGVAR(core,magicRefuel)) and (_timer <= 24)) then { {if (((side _x) getFriend (side _unitG)) >= 0.6) then {_x setFuel 1; if (isPlayer _x) then {"Vehicle Refueled" remoteExec ["hint", _x]};}} forEach ((vehicle (leader _unitG)) nearEntities [["Air", "LandVehicle"], 100]);};
...
if (_request) then {[_cis] remoteExecCall ["hal_common_fnc_reqLogisticsDeleteActions"]};
_cis setVariable ["HAL_Requested",false,true];
...
if (((fuel _Trg) > 0.1) or ((damage _Trg) >= 0.9) or (isNull (group (assignedDriver (_this select 1))))) then {_dried = _dried - [_Trg]};
if ((_HQ getVariable [QEGVAR(common,debug),false]) or (isPlayer (leader _unitG))) then {deleteMarker ("markFuelSupp" + str (_unitG))};
_lastOne = true;
...
if (_lastOne) then
	{
	_fSupp = _HQ getVariable [QEGVAR(core,fSupportedG),[]];
	_fSupp = _fSupp - [(group _Trg)];
	_HQ setVariable [QEGVAR(core,fSupportedG),_fSupp]
	};
