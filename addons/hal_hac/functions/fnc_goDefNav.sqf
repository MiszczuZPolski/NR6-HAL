...
// Originally from nr6_hal/HAL/GoDefNav.sqf
_SCRname = "GoDefNav";
...
if not (_isWater) exitWith {_defSpot = _HQ getVariable [QEGVAR(core,defSpot),[]];
	_defSpot = _defSpot - [_unitG];
	_HQ setVariable [QEGVAR(core,defSpot),_defSpot];
	_def = _HQ getVariable [QEGVAR(core,def),[]];
	_def = _def - [_unitG];
...
[_unitG,[_posX,_posY,0],"HQ_ord_defendRNav",_HQ] call EFUNC(common,orderPause);
...
_task = [(leader _unitG),["Patrol towards the designated area and standby for further orders. ", "Patrol Waters And Standby", ""],_DefPos,"defend"] call EFUNC(common,addTask);
