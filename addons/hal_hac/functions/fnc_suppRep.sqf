...
_rep = EGVAR(hal_data,rep) + EGVAR(hal_data,wS_rep) - RHQs_Rep;
_noenemy = true;
...
_repSG = [];
	...
	if not (_x in _repS) then
		...
		if ((toLower (typeOf (assignedVehicle _x))) in _rep) then 
			{
			_repS pushBack _x;
			if not ((group _x) in (_repSG + (_HQ getVariable [QEGVAR(hal_hac,specForG),[]]) + (_HQ getVariable [QEGVAR(core,cargoOnly),[]]))) then 
				{
