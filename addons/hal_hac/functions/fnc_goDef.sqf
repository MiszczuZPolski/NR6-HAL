...
// Originally from nr6_hal/HAL/GoDef.sqf
_SCRname = "GoDef";
...
if ((_unitG in ((_HQ getVariable [QEGVAR(core,nCCargoG),[]]) - (_HQ getVariable [QEGVAR(core,airG),[]]))) and ((count (units _unitG)) <= 1) or (_unitG in ((_HQ getVariable [QEGVAR(core,supportG),[]]) - (_HQ getVariable [QEGVAR(core,airG),[]])))) then
	{
...
if (_HQ getVariable [QEGVAR(common,debug),false]) then
	...
	_clr = "ColorBrown";
	_txt = "LMCU " + _signum;
	if (_unitG in ((_HQ getVariable [QEGVAR(core,supportG),[]]) - (_HQ getVariable [QEGVAR(core,airG),[]]))) then {_clr = "ColorKhaki";_txt = "DSupp " + _signum};
	_pltxt = " - DEFEND POSITION";
	if (_unitG in ((_HQ getVariable [QEGVAR(core,supportG),[]]) - (_HQ getVariable [QEGVAR(core,airG),[]]))) then {_pltxt = " - SUPPORT"};

	_i = [_DefPos,_unitG,"markDef",_clr,"ICON","waypoint",_txt,_pltxt,[0.5,0.5]] call EFUNC(common,mark)
	};
...
_task = [(leader _unitG),["Take a defensive position at the designated coordinates.", "Defend Position", ""],_DefPos,"defend"] call EFUNC(common,addTask);
...
//_wp = [_unitG,_DefPos,_tp,"SAFE","YELLOW","NORMAL",["true","deletewaypoint [(group this), 0];"],true,0,[0,0,0],_formation] call EFUNC(common,WPadd);
...
if not (isPlayer _UL) then
	{
		if ((random 100) < EGVAR(core,aIChatDensity)) then
			{[_UL,GVAR(aIC_OrdEnd),"OrdEnd"] call EFUNC(common,AIChatter)}
	};
