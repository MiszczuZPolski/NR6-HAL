...
// Originally from nr6_hal/HAL/GoAttSniper.sqf
...
if (_unitG in (_HQ getVariable [QEGVAR(core,airG),[]])) then {_distance2 = 800};
...
_dstMpl = (_HQ getVariable [QEGVAR(core,attSnpDistance),1]) * (_unitG getVariable [QGVAR(myAttDst),1]);
...
if (_isWater) exitWith
	...
	[_Trg,"SnpAttacked"] call EFUNC(common,varReductor)
	};

_positions = [[_posX,_posY,1],(getPosATL _Trg),200,1,1,_unitG] call EFUNC(common,findOverwatchPos);
...
if (_HQ getVariable [QEGVAR(common,debug),false]) then
	...
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","waypoint","SNP " + (groupId _unitG) + " " + _signum," - ATTACK",[0.5,0.5]] call EFUNC(common,mark)
	};
...
if ((isNull _AV) and (([_posX,_posY] distance _UL) > EGVAR(core,cargoObjRange)) and not (isPlayer (leader _unitG))) then
	{
	...
	_wp0 = [_unitG,[(_posX + _LX)/2,(_posY + _LY)/2],"MOVE","SAFE","YELLOW",_spd,["true","deletewaypoint [(group this), 0];"],true,0.001,_TO] call EFUNC(common,WPadd);

	_nW = 2;
	};

_task = [(leader _unitG),["Take out the designated hostile targets", "Neutralize Hostile Targets", ""],[_posX,_posY],"target"] call EFUNC(common,addTask);
...
if (not (isNull _AV) and (_GDV in (_HQ getVariable [QEGVAR(core,airG),[]]))) then {_beh = "CARELESS"};
_spd = "NORMAL";
...
_wp = [_gp,_pos,_tp,_beh,"YELLOW",_spd,_sts,_crr,0.001,_TO] call EFUNC(common,WPadd);

if ((EGVAR(core,synchroAttack)) and not (isPlayer (leader _unitG))) then
	{
...
if (_unitG in (_HQ getVariable [QEGVAR(core,nCrewInfG),[]])) then {_pass orderGetIn false};
sleep 5;
_alive = true;
if (_halfway) then
	{
	...
	_wp = [_unitG,[_posX,_posY],"MOVE","STEALTH","YELLOW","NORMAL",["true","deletewaypoint [(group this), 0];"],true,0.001,[0,0,0],_formation] call EFUNC(common,WPadd);
...
if not (_alive) exitWith {_unitG setVariable [("Busy" + (str _unitG)),false];[_Trg,"SnpAttacked"] call EFUNC(common,varReductor)};
...
_wp = [_unitG,getPosATL (vehicle (leader _unitG)),"SENTRY","STEALTH","RED","NORMAL",["true",""],_cur,0.001,[0,0,0],_formation] call EFUNC(common,WPadd);

_fEH = (leader _unitG) addEventHandler ["Fired",{[_this,EFUNC(common,fireCount)] call EFUNC(common,spawn)}];
...
if not (_request) then {_unitG setVariable [QEGVAR(common,waitingTarget),_trg]};
_cause = [_unitG,5,true,0,240,[],false,true,true,false,false,false,true] call EFUNC(common,wait);
_timer = _cause select 0;
_alive = _cause select 1;
...
if (_timer > 240) then {[_unitG] call CBA_fnc_clearWaypoints};
...
_wp = [_unitG,[_wPosX,_wPosY],"MOVE","STEALTH","GREEN","NORMAL",["true","deletewaypoint [(group this), 0];"],true,0,[0,0,0],_formation] call EFUNC(common,WPadd);
if not (_request) then {_unitG setVariable [QEGVAR(common,waitingTarget),_trg]};
_cause = [_unitG,6,true,0,300,[],false] call EFUNC(common,wait);
_timer = _cause select 0;
_alive = _cause select 1;

if not (_alive) exitWith {_unitG setVariable [("Busy" + (str _unitG)),false];[_Trg,"SnpAttacked"] call EFUNC(common,varReductor);if ((_HQ getVariable [QEGVAR(common,debug),false]) or (isPlayer (leader _unitG))) then {deleteMarker ("markAttack" + str (_unitG))}};
