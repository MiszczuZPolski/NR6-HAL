...
// Originally from nr6_hal/HAL/GoRepSupp.sqf
...
_RepPoints = _HQ getVariable [QEGVAR(core,repPoints),[]];
...
[_unitG,[_posX,_posY,0],"HQ_ord_repS",_HQ] call EFUNC(common,orderPause);
...
if (_HQ getVariable [QEGVAR(common,debug),false]) then
	...
	_i = [[_posX,_posY],_unitG,"markRepSupp","ColorKhaki","ICON","waypoint","REPAIR " + (groupId _unitG) + " " + _signum," - REPAIR SUPPORT",[0.5,0.5],180] call EFUNC(common,mark)
	};

_task = [(leader _unitG),["Perform field repairs for " + (groupId (group _Trg)), "Repair " + (groupId (group _Trg)), ""],(position _Trg),"repair"] call EFUNC(common,addTask);
...
if (_request) then {
	...
	[_rtr,"Repair"] remoteExecCall ["hal_common_fnc_reqLogisticsActions"];
...
while {(_counter <= 3)} do
	...
	if not (_alive) exitWith
		...
		_HQ setVariable [QEGVAR(core,repPoints),_RepPoints];
	...
	if ((EGVAR(core,magicRepair)) and (_timer <= 24)) then { {if (((side _x) getFriend (side _unitG)) >= 0.6) then {_x setDamage 0; if (isPlayer _x) then {"Vehicle Repaired" remoteExec ["hint", _x]};}} forEach ((vehicle (leader _unitG)) nearEntities [["Air", "LandVehicle"], 100]);};
...
if (_request) then {[_rtr] remoteExecCall ["hal_common_fnc_reqLogisticsDeleteActions"]};
_rtr setVariable ["HAL_Requested",false,true];
...
if (_HQ getVariable [QEGVAR(core,supportRTB),false]) then {_pos = _startpos; if not (isNull (_HQ getVariable [QEGVAR(core,supportDecoy),objNull])) then {if ((random 100) <= (_HQ getVariable [QEGVAR(core,sDChance),100])) then {_pos = (getPos (_HQ getVariable [QEGVAR(core,supportDecoy),objNull]))}}; _RepPoints = _RepPoints - [_Trg]; if not (_task isEqualTo taskNull) then {[_task,(leader _unitG),["Return to base.", "Return To Base", ""],_pos,"ASSIGNED",0,false,true] call BIS_fnc_SetTask;}};
...
_wp = [_unitG,_pos,_tp,"SAFE","BLUE","FULL",["true","{(vehicle _x) land 'LAND'} foreach (units (group this));deletewaypoint [(group this), 0]" + _radd]] call EFUNC(common,WPadd);
...
_RepPoints = _RepPoints - [_Trg];
_HQ setVariable [QEGVAR(core,repPoints),_RepPoints];
if not (_HQ getVariable [QEGVAR(core,supportRTB),false]) then {if not (_task isEqualTo taskNull) then {[_task,"SUCCEEDED",true] call BIS_fnc_taskSetState}};

_rtr enableAI "TARGET";_rtr enableAI "AUTOTARGET";
_unitG setVariable [("Busy" + _unitvar), false];

if (((damage _Trg) < 0.1) or ((damage _Trg) >= 0.9) or (isNull (group (assignedDriver (_this select 1))))) then {_damaged = _damaged - [_Trg]};
if ((_HQ getVariable [QEGVAR(common,debug),false]) or (isPlayer (leader _unitG))) then {deleteMarker ("markRepSupp" + str (_unitG))};
_lastOne = true;
...
if (_lastOne) then
	{
	_rSupp = _HQ getVariable [QEGVAR(core,rSupportedG),[]];
	_rSupp = _rSupp - [(group _Trg)];
	_HQ setVariable [QEGVAR(core,rSupportedG),_rSupp]
	};
