...
_Armor = (_HQ getVariable [QEGVAR(hal_boss,lArmorG),[]]) + (_HQ getVariable [QEGVAR(hal_boss,hArmorG),[]]);

if (_unitG in _Armor) then {_distance2 = 500};
if (_unitG in (_HQ getVariable [QEGVAR(core,airG),[]])) then {_distance2 = 750};

_dstMpl = (_HQ getVariable [QEGVAR(core,attInfDistance),1]) * (_unitG getVariable [QGVAR(myAttDst),1]);
...
if (_isWater) exitWith
	...
	if not (_request) then {[_trg,"InfAttacked"] call EFUNC(common,varReductor)};
	};

[_unitG,[_posX,_posY,0],"HQ_ord_attack",_HQ] call EFUNC(common,orderPause);
...
if (_HQ getVariable [QEGVAR(common,debug),false]) then
	...
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","waypoint", "INF " + (groupId _unitG) + " " + _signum," - ATTACK",[0.5,0.5]] call EFUNC(common,mark)
	};
...
if (not (isNull _AV) and ((_HQ getVariable ["RydHQ_CargoFind",0]) > 0) and not (_GDV == _unitG) and not (_IsAPlayer)) then
	{
	_task = [(leader _unitG),["Embark your lift to your target", "Get In Lift", ""],(getPosATL (leader _unitG)),"getin"] call EFUNC(common,addTask);
...
if ((({alive _x} count (units _unitG)) < 1) or (_timer > 300)) exitwith {
	if not (_request) then {[_trg,"InfAttacked"] call EFUNC(common,varReductor)};if ((_HQ getVariable ["RydHQ_Debug",false]) or (isPlayer (leader _unitG))) then {deleteMarker ("markAttack" + str (_unitG))};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 0];_GDV setVariable [("Busy" + _unitvar), false];};
...
if not (_alive) exitWith {_unitG setVariable [("Busy" + (str _unitG)),false];if not (_request) then {[_trg,"InfAttacked"] call EFUNC(common,varReductor)}};
...
if not (_request) then {
	if not (isPlayer (leader _unitG)) then {
		_wp = [_unitG,_trg,"SAD",_beh,"YELLOW",_spd,["true","deletewaypoint [(group this), 0];"],_cur,0,[0,0,0],_formation] call EFUNC(common,WPadd);
...
if not (_request) then {_unitG setVariable [QEGVAR(common,waitingTarget),_trg]};
if not (_isAPlayer) then {_unitG setVariable ["InfGetinCheck" + (str _unitG),true]};
_cause = [_unitG,6,true,0,30,[(_HQ getVariable [QEGVAR(core,airG),[]]),(_HQ getVariable [QEGVAR(common,knEnemiesG),[]]),_HQ],false] call EFUNC(common,wait);
_timer = _cause select 0;
_alive = _cause select 1;
...
if ((_unitG in (_HQ getVariable [QEGVAR(core,garrison),[]])) and not (isPlayer (leader _unitG))) then
	...
	_wp = [_unitG,_Spos,"MOVE","AWARE","YELLOW","NORMAL",["true","deletewaypoint [(group this), 0];"],true,5] call EFUNC(common,WPadd);
