...
// Originally from nr6_hal/HAL/GoCaptureNaval.sqf
...
[_unitG,[_posX,_posY,0],"HQ_ord_captureNav",_HQ] call EFUNC(common,orderPause);
...
_task = [(leader _unitG),["Secure the objective. Neutralize any hostile ships and hold the objective.", "Secure And Hold Objective", ""],[_posX,_posY],"move"] call EFUNC(common,addTask);
...
forEach (_HQ getVariable [QEGVAR(core,navalG),[]]) - (((_HQ getVariable [QEGVAR(core,airG),[]]) - (_HQ getVariable [QEGVAR(core,nCrewInfG),[]])) + (_HQ getVariable [QGVAR(staticG),[]]) + (_HQ getVariable [QEGVAR(core,supportG),[]]) + (_HQ getVariable [QEGVAR(core,artG),[]]) + ((_HQ getVariable [QEGVAR(core,nCCargoG),[]]) - ((_HQ getVariable [QEGVAR(core,nCrewInfG),[]]) - (_HQ getVariable [QEGVAR(core,supportG),[]]))));
