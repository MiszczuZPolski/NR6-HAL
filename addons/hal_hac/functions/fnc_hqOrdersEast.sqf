...
_noCombat = (_HQ getVariable [QEGVAR(hal_boss,nCAirG),[]]) + (_HQ getVariable [QEGVAR(core,nCCargoG),[]]) + (_HQ getVariable [QGVAR(ammoSupportG),[]]) + (_HQ getVariable [QGVAR(repSupportG),[]]) + (_HQ getVariable [QGVAR(medSupportG),[]]) + (_HQ getVariable [QGVAR(fuelSupportG),[]]);
_airRecon = (_HQ getVariable [QGVAR(rAirG),[]]);
_armored = (_HQ getVariable [QEGVAR(hal_boss,hArmorG),[]]) - _noCombat; 
_mechanized = ((_HQ getVariable [QGVAR(mArmorG),[]]) + (_HQ getVariable [QEGVAR(hal_boss,lArmorG),[]]) + (_HQ getVariable [QEGVAR(hal_boss,carsG),[]])) - _noCombat;
