...
// Originally from nr6_hal/HAL/GoFlank.sqf
_SCRname = "GoFlank";
...
_dstMpl = (_HQ getVariable [QEGVAR(core,flankDistance),1]) * (_unitG getVariable [QGVAR(myAttDst),1]);
...
if (_isWater) then {_unitG setVariable [("Busy" + (str _unitG)),false];_HQ setVariable [QGVAR(flankAv),(_HQ getVariable [QGVAR(flankAv),[]]) - [_unitG]]};
...
if (_isWater) exitWith {_unitG setVariable [("Busy" + (str _unitG)),false];_HQ setVariable [QGVAR(flankAv),(_HQ getVariable [QGVAR(flankAv),[]]) - [_unitG]]};
...
if ((_ammo > 0) and not (_busy)) then
	...
	[_unitG,[_posXWP4,_posYWP4,0],"HQ_ord_flank",_HQ] call EFUNC(common,orderPause);
	...
	if (_HQ getVariable [QEGVAR(common,debug),false]) then
		...
		_i1 = [[_posXWP1,_posYWP1],_unitG,"markFlank1","ColorOrange","ICON","waypoint","FLANK1 "  + (groupId _unitG) + " " + _signum," - FLANK 1",[0.5,0.5]] call EFUNC(common,mark);
		_i2 = [[_posXWP2,_posYWP2],_unitG,"markFlank2","ColorOrange","ICON","waypoint","FLANK2 "  + (groupId _unitG) + " " + _signum," - FLANK 2",[0.5,0.5]] call EFUNC(common,mark);
		_i3 = [[_posXWP3,_posYWP3],_unitG,"markFlank3","ColorOrange","ICON","waypoint","FLANK3 "  + (groupId _unitG) + " " + _signum," - FLANK 3",[0.5,0.5]] call EFUNC(common,mark);
		_i4 = [[_posXWP4,_posYWP4],_unitG,"markFlank4","ColorOrange","ICON","waypoint","FLANK4 "  + (groupId _unitG) + " " + _signum," - FLANKING ATTACK",[0.5,0.5]] call EFUNC(common,mark)
		};
	...
	if (((_HQ getVariable [QEGVAR(core,cargoFind),0]) > 0) and not (_IsAPlayer) and (isNull _AV) and (([_posXWP4,_posYWP4] distance (vehicle _UL)) > EGVAR(core,cargoObjRange))) then
		{
	...
	if not (_alive) exitWith
		{
		_unitG setVariable [("Busy" + (str _unitG)),false]; if ((_HQ getVariable [QEGVAR(common,debug),false]) or (isPlayer (leader _unitG))) then {{deleteMarker _x} forEach [_i1,_i2,_i3,_i4]}
		};
	...
	if (not (isNull _AV) and ((_HQ getVariable [QEGVAR(core,cargoFind),0]) > 0) and not (_GDV == _unitG) and not (_IsAPlayer)) then
		{
		_task = [(leader _unitG),["Wait for your lift to your destination.", "Get In Lift", ""],(getPosATL (leader _unitG)),"getin"] call EFUNC(common,addTask);
	...
	_task = [(leader _unitG),["Flank hostile forces and standby for further orders.", "Take Flanking Position", ""],[_posXWP1,_posYWP1],"navigate"] call EFUNC(common,addTask);

	_Ctask = taskNull;
	if (not ((leader _GDV) == (leader _unitG)) and not (_GDV == _unitG)) then
		{
		_Ctask = [(leader _GDV),["", "Provide Lift", ""],[_posXWP1,_posYWP1],"navigate"] call EFUNC(common,addTask)
		};
	...
	if not (isPlayer (leader _grp)) then {_formation = "STAG COLUMN"};
	...
	if (({alive _x} count (units _unitG)) < 1) exitWith {_unitG setVariable [("Busy" + (str _unitG)),false];if ((_HQ getVariable [QEGVAR(common,debug),false]) or (isPlayer (leader _unitG))) then {{deleteMarker _x} forEach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];
	}};
	...
	if not (_IsAPlayer) then {
		if not (((group _DAV) == (group _UL)) or (isNull (group _DAV))) then
			...
			_cause = [_GDV,6,true,400,30,[(_HQ getVariable [QEGVAR(core,airG),[]]),(_HQ getVariable [QEGVAR(common,knEnemiesG),[]])],true] call EFUNC(common,wait);
			_timer = _cause select 0;
			_alive = _cause select 1;
			_enemy = _cause select 2;
			}
	...
	_UL = leader _unitG;if not (isPlayer _UL) then {if ((_timer <= 30) and not (_enemy)) then {if ((random 100) < EGVAR(core,aIChatDensity)) then {[_UL,GVAR(aIC_OrdFinal),"OrdFinal"] call EFUNC(common,AIChatter)}}};
	...
	_wp4 = [_unitG,[_posXWP4,_posYWP4],"SAD",_beh,"RED",_spd,["true","deletewaypoint [(group this), 0];"],true,200,[0,0,0],_formation] call EFUNC(common,WPadd);
	...
	_HQ setVariable [QGVAR(flankAv),(_HQ getVariable [QGVAR(flankAv),[]]) - [_unitG]];
	_unitG setVariable [("Busy" + _unitvar), false];

	if ((_HQ getVariable [QEGVAR(common,debug),false]) or (isPlayer (leader _unitG))) then {_i4 setMarkerColor "ColorBlue"};
	sleep 30;
	if ((_HQ getVariable [QEGVAR(common,debug),false]) or (isPlayer (leader _unitG))) then
		{
			{
