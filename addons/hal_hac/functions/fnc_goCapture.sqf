...
// Originally from nr6_hal/HAL/GoCapture.sqf
_SCRname = "GoCapture";
...
_currentObj = _HQ getVariable [QEGVAR(core,nObj),1];
...
_dstMpl = (_HQ getVariable [QEGVAR(core,captureDistance),1]) * (_unitG getVariable [QGVAR(myAttDst),1]);
...
[_unitG,[_posX,_posY,0],"HQ_ord_capture",_HQ] call EFUNC(common,orderPause);
...
if (_HQ getVariable [QEGVAR(common,debug),false]) then
	...
	_i = [[_posX,_posY],_unitG,"markCapture","ColorRed","ICON","waypoint","CAP " + (groupId _unitG) + " " + _signum," - SECURE AREA",[0.5,0.5]] call EFUNC(common,mark)
	};
...
_eClose1 = [[_posX,_posY],(_HQ getVariable [QEGVAR(common,knEnemiesG),[]]),300] call EFUNC(common,closeEnemyB);
...
if not (isNull _NeN) then
	{
	_eClose2 = [_UL,(_HQ getVariable [QEGVAR(common,knEnemiesG),[]]),300] call EFUNC(common,closeEnemyB);
	_tooC2 = _eClose2 select 0;
	_dstEM2 = _eClose2 select 1;
	_eClose3 = [(leader _HQ),(_HQ getVariable [QEGVAR(common,knEnemiesG),[]]),300] call EFUNC(common,closeEnemyB);
...
if (not (isNull _GDV) and (_GDV in ((_HQ getVariable [QEGVAR(core,nCCargoG),[]]) + (_HQ getVariable [QEGVAR(core,airG),[]]))) and (_NeNMode) and not (isPlayer (leader _GDV))) then {_LX1 = (getPosATL _UL) select 0;_LY1 = (getPosATL _UL) select 1;_halfway = true};
...
//	_spd = "LIMITED";
	...
	_wp0 = [_unitG,[_posX,_posY],"MOVE",_beh,"YELLOW",_spd,["true","deletewaypoint [(group this), 0];"],true,0,_TO] call EFUNC(common,WPadd);
...
//		if not ((_HQ getVariable ["RydHQ_Order","ATTACK"]) == "DEFEND") then {_unitG setVariable [("Busy" + _unitvar), false];} else {_unitG setVariable [("Busy" + _unitvar), true];};
		...
		if (not (isNull _AV) and ((_HQ getVariable [QEGVAR(core,cargoFind),0]) > 0) and not (_GDV == _unitG) and not (_IsAPlayer)) then
			...
			if ((({alive _x} count (units _unitG)) < 1) or (_timer2 > 300)) exitWith
				...
				_Trg setVariable [("Capturing" + (str _Trg) + (str _HQ)),[_isAttacked,_amountC]];
				if ((_HQ getVariable [QEGVAR(common,debug),false]) or (isPlayer (leader _unitG))) then {deleteMarker ("markCapture" + str (_unitG))};
				if not (isNull _GDV) then
					{
					[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 0];
					_GDV setVariable [("CargoM" + (str _GDV)), false];
...
_task = [(leader _unitG),["Secure the objective. Neutralize any hostile forces and hold the objective.", "Secure And Hold Objective", ""],[_posX,_posY],"move"] call EFUNC(common,addTask);
...
_wp = [_gp,_pos,_tp,_beh,"YELLOW",_spd,_sts,_crr,0,_TO] call EFUNC(common,WPadd);
...
if not (_IsAPlayer) then {
	if not (((group _DAV) == (group _UL)) or (isNull (group _DAV))) then
		...
		_cause = [_GDV,6,true,300,30,[(_HQ getVariable [QEGVAR(core,airG),[]]),(_HQ getVariable [QEGVAR(common,knEnemiesG),[]]),_HQ],false] call EFUNC(common,wait);
		_timer = _cause select 0;
		...
		_unitG setVariable [QEGVAR(common,waitingObjective),[_HQ,_trg]];
		_cause = [_unitG,6,true,300,30,[(_HQ getVariable [QEGVAR(core,airG),[]]),(_HQ getVariable [QEGVAR(common,knEnemiesG),[]]),_HQ],false] call EFUNC(common,wait);
		_timer = _cause select 0;
		_alive = _cause select 1;
		_enemy = _cause select 2;
		};
...
if (((_halfway) or (_earlyD)) and not (_IsAPlayer)) then
	...
	_wp = [_unitG,[_posX,_posY],"MOVE","AWARE","YELLOW","NORMAL",["true","deletewaypoint [(group this), 0];"],true,0,[0,0,0],_formation] call EFUNC(common,WPadd);
...
_wp = [_unitG,_Trg,"SAD",_beh,"RED",_spd,["true","deletewaypoint [(group this), 0];"],true,100,[0,0,0],_formation] call EFUNC(common,WPadd);
...
_isTaken = false;
_waitTime = _HQ getVariable [QEGVAR(core,objHoldTime),600];
...
waitUntil
	...
	if (not (_BBProgN > _BBProg)) then
		...
		_AllV0 = _Trg nearEntities [["AllVehicles"],_HQ getVariable [QEGVAR(core,objRadius1),300]];
		...
		_Civs0 = _Trg nearEntities [["Civilian"],_HQ getVariable [QEGVAR(core,objRadius1),300]];
		...
		_AllV20 = _Trg nearEntities [["AllVehicles"],_HQ getVariable [QEGVAR(core,objRadius2),500]];
		...
		if (_trg in (_HQ getVariable [QEGVAR(common,taken),[]])) then {_isTaken = true};

		if (not (_HQ getVariable [QEGVAR(core,unlimitedCapt),false]) and (_NearAllies >= (_HQ getVariable [QEGVAR(core,captLimit),10])) and (_NearEnemies <= (0 + (((_HQ getVariable [QEGVAR(core,recklessness),0.5])/(0.5 + (_HQ getVariable [QEGVAR(core,consistency),0.5])))*10)))) then
			{
			if (((_HQ getVariable ["BBProgress","nope"]) isEqualTo "nope") and not (_HQ getVariable [QEGVAR(core,simpleMode),false])) then {_HQ setVariable [QEGVAR(core,nObj),(_HQ getVariable [QEGVAR(core,nObj),1]) + 1];};
			if ((random 100) < (15*(_HQ getVariable [QEGVAR(core,nObj),1]))) then
				{
				_HQ setVariable [QEGVAR(core,flankingDone),false];
				_HQ setVariable [QEGVAR(core,flankingInit),false];
				_HQ setVariable [QGVAR(flankingTimeStamp),time];
				};

			_HQ setVariable [QEGVAR(core,morale),(_HQ getVariable [QEGVAR(core,morale),0]) + _mGain];
...
if ((_HQ getVariable [QEGVAR(core,unlimitedCapt),false]) or not (_isTaken)) then {_Trg setVariable [("Capturing" + (str _Trg) + (str _HQ)),[0,0]]};
...
_all = true;
	...
	{
	if ((_x in (_HQ getVariable [QGVAR(attackAv),[]])) and not (_x getVariable ("Busy" + str (_x)))) exitWith {_all = false};
	}
forEach (_HQ getVariable [QEGVAR(core,friends),[]]) - (((_HQ getVariable [QEGVAR(core,airG),[]]) - (_HQ getVariable [QEGVAR(core,nCrewInfG),[]])) + (_HQ getVariable [QEGVAR(core,navalG),[]]) + (_HQ getVariable [QGVAR(staticG),[]]) + (_HQ getVariable [QEGVAR(core,supportG),[]]) + (_HQ getVariable [QEGVAR(core,artG),[]]) + ((_HQ getVariable [QEGVAR(core,nCCargoG),[]]) - ((_HQ getVariable [QEGVAR(core,nCrewInfG),[]]) - (_HQ getVariable [QEGVAR(core,supportG),[]]))));
