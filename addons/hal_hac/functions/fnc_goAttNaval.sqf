...
// Originally from nr6_hal/HAL/GoAttNaval.sqf
_SCRname = "GoAttNaval";
...
[_unitG,[_posX,_posY,0],"HQ_ord_attackNaval",_HQ] call EFUNC(common,orderPause);
...
if (_HQ getVariable [QEGVAR(common,debug),false]) then
	...
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","waypoint","NAVATT " + (groupId _unitG) + " " + _signum," - ATTACK",[0.5,0.5]] call EFUNC(common,mark);
	};

_task = [(leader _unitG),["Engage the designated hostile ships. ROE: WEAPONS FREE.", "Engage Hostile Ship(s)", ""],[_posX,_posY],"attack"] call EFUNC(common,addTask);
...
if (EGVAR(core,synchroAttack)) then
	{
	if not (_request) then {[_wp,_Trg] call EFUNC(common,WPSync)};
...
if not (_alive) exitWith
	...
	if not (_request) then {[_Trg,"NavAttacked"] call EFUNC(common,varReductor)}
	};
...
_cur = true;
if (EGVAR(core,synchroAttack)) then {_cur = false};
