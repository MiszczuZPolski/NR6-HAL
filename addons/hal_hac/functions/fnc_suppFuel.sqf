...
_fuel = EGVAR(hal_data,fuel) + EGVAR(hal_data,wS_fuel) - RHQs_Fuel;
_noenemy = true;
...
_fuelSG = [];
	...
	if not (_x in _fuelS) then
		...
		if ((toLower (typeOf (assignedVehicle _x))) in _fuel) then 
			{
			_fuelS pushBack _x;
			if not ((group _x) in (_fuelSG + (_HQ getVariable [QEGVAR(hal_hac,specForG),[]]) + (_HQ getVariable [QEGVAR(core,cargoOnly),[]]))) then 
				{
