...
RYD_WPadd =
	...
	if not (_rds == 0.01) then {[_group] call CBA_fnc_clearWaypoints};
...
RYD_DistOrdD =
	...
	forEach _first;
		...
		if ((count _mid) >= _pos) then {while {sleep 0.1; not (isNil {_mid select _pos})} do {_pos = _pos + 1}};
		_mid set [_pos,_x];
		}
	forEach _sort;
...
RYD_VarReductor =
	...
	_navEnough = _HAC_Attacked select 4;
