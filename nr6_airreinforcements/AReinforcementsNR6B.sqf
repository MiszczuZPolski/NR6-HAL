...
while {true} do
    ...
    {
        if ((alive _x) and ((damage _x) < 0.4) and !(_x getVariable ["Air_ReinforcementsNR6_Taken",false])) then {
    ...
    {
        if ( !([] isEqualTo ((getPos _x) findEmptyPosition [0,0,"B_Heli_Transport_01_F"])) and (alive _x)) then {
    ...
    if ((_CurrentForces) < (_Threshold*_CStartForces)) then
        {
            if ( !(_GoodsideForces isEqualTo []) and !(_GoodPads isEqualTo [])) then {
